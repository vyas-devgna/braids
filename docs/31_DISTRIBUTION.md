...
| Everything else | `npx braids-skill@next <host>` |
...
Package name `braids-skill`. It exists to serve `npx braids-skill@next <host>` for hosts with no plugin system, and to give the skills a versioned artifact. It is not a library: there is no runtime import surface, and `bin` points at a file copier.

Prereleases use the `next` dist-tag; stable versions use `latest`. If npm is unavailable, `npx github:vyas-devgna/braids <host>` runs the same installer directly from the public repository.
...
Step 3 becomes defensible once the 100-case suite is graded on at least two hosts and the adapters for those hosts reach `tested`. Step 4 additionally needs the `docs/22` per-host minimums.
